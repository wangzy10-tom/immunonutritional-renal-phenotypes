...
input_path <- file.path(
  project_root, "output", "nhanes_albumin_benchmarks",
  "NHANES_albumin_benchmark_results.rds"
)
output_dir <- file.path(project_root, "output", "nhanes_corrected_bootstrap")
...
primary_formula <- stats::as.formula(paste(
  "survival::Surv(PERMTH_INT, MORTSTAT) ~ phenotype_total_protein +", base_terms
))
...
ph_table <- dplyr::bind_rows(
  extract_ph(primary_fit, "Total-protein phenotype"),
...
original_p1 <- tibble::tibble(
  ...
  HR = exp(original_co["phenotype_total_proteinP1", "coef"]),
  lower_95 = exp(original_ci["phenotype_total_proteinP1", 1]),
  upper_95 = exp(original_ci["phenotype_total_proteinP1", 2]),
  p_value = original_co["phenotype_total_proteinP1", "Pr(>|z|)"]
)
