...
output_dir <- file.path(root, "output", "nhanes_albumin_amendment17_2026-08-30/selection_bias")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

eligible <- readRDS(eligible_path) |>
  dplyr::mutate(
    selected = as.integer(is.finite(NLR) & NLR > 0 & is.finite(SII) & SII > 0 & is.finite(LBXHGB) & is.finite(LBXSAL) & LBXSAL > 0 & is.finite(BMXBMI) & is.finite(LBXSCR) & LBXSCR > 0),
...
core_features <- c("NLR", "SII", "LBXHGB", "LBXSAL", "BMXBMI", "LBXSCR")
...
extract_phenotype <- function(fit, model_label) {
  ...
  terms <- names(beta)
  keep <- grepl("^phenotype_albumin", terms)
  tibble::tibble(
    model = model_label,
