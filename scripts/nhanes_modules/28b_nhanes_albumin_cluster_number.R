...
input_path <- paste0(
  "./output/nhanes_2011_2018_rebuild/",
  "NHANES_2011_2018_rebuilt_analytical_cohort.rds"
)
output_dir <- "./output/nhanes_albumin_amendment17_2026-08-30/cluster_number_validation"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

dat <- readRDS(file.path("output", "nhanes_albumin_amendment17_2026-08-30", "INTERNAL_NHANES_albumin_complete_cohort.rds"))
...
label_by_profile <- function(kmeans_cluster, k) {
  ...
  profile_matrix <- data.frame(
    ...
    negative_albumin = -profiles$albumin,
