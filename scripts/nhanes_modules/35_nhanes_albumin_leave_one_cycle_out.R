...
assignment_path <- file.path(
  ...
  "NHANES_albumin_benchmark_assignments.csv"
)
output_dir <- file.path(root, "output", "nhanes_albumin_amendment17_2026-08-30/leave_one_cycle_out")
...
fit_and_project <- function(train, test) {
  raw_variables <- c("NLR", "SII", "LBXHGB", "LBXSAL", "BMXBMI", "LBXSCR")
  ...
  transform_matrix <- function(dat) {
    cbind(
      ...
      albumin = apply_winsor(dat$LBXSAL, limits$LBXSAL),
  ...
  p2_score <- -z_small(remaining$haemoglobin) - z_small(remaining$albumin) -
...
cohort <- readRDS(file.path("output", "nhanes_albumin_amendment17_2026-08-30", "INTERNAL_NHANES_albumin_complete_cohort.rds")) |> dplyr::select(-dplyr::any_of("phenotype_albumin"))
...
dat <- cohort |>
  ...
  dplyr::mutate(
    phenotype_full = factor(phenotype_albumin, levels = c("P3", "P2", "P1")),
