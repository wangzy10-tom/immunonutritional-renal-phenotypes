...
output_dir <- file.path(root, "output", "nhanes_albumin_amendment17_2026-08-30/absolute_risk_RMST")
...
model_data <- readRDS(input_path)$model_data |>
  dplyr::mutate(
    phenotype_albumin = factor(
      phenotype_albumin, levels = phenotype_levels
    ),
...
standardise_survival <- function(fit, standardisation_data, standardisation_weights) {
  ...
  for (i in seq_along(phenotype_levels)) {
    ...
    counterfactual$phenotype_albumin <- factor(
...
weighted_cox_check <- tibble::tibble(
  ...
  HR = exp(primary_beta[c("phenotype_albuminP2", "phenotype_albuminP1")]),
  frozen_svycoxph_HR = c(1.4875564163192825, 2.081825284902306)
) |>
